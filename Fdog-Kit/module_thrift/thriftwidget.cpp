...
{
    qDebug() << "handleI64 paramName = " << paramName;
    QString value = str.mid(0, 16);
    str = str.mid(16);
    QString end = ",";
    isEnd = str.mid(0,2);
...
QString thriftwidget::getServerInterface(QString &fileContent) {
    ...
    if (index) {
        ...
        QIcon icon2(":/module_thrift/images/light/server.png");
...
void thriftwidget::getStructInfo(QString &fileContent) {
    while(true) {
        if (fileContent.contains("struct")) {
            ...
            qDebug() << "structParamMap.size() = " << structParamMap.size() << " structParamMap = " << &structParamMap;
...
void thriftwidget::comboBox_testType_ChangedIndex(int index) {
    ui->comboBox_testType->setCurrentIndex(index);
    on_comboBox_testType_currentIndexChanged(index);
}
...
void thriftwidget::on_toolButton_close_clicked()
{
    ui->label_time->setText("");
    ui->toolButton_close->hide();
}
